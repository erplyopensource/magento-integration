...
class Eepohs_Erply_Model_Product_Import extends Eepohs_Erply_Model_Erply
{
    ...
    public function getTotalRecords($storeId)
    {
        ...
        $results = $this->makeRequest('getProducts');
    ...
    public function importProducts()
    {
        ...
        if ($queue) {
            ...
            for ($i = $firstPage; $i <= ($firstPage + $loops); $i++) {
                ...
                $result = $this->makeRequest('getProducts');
                $return = "";
                Mage::helper('Erply')->log("Erply product import:");
                Mage::helper('Erply')->log(print_r($result, 1));

                $start = time();
                foreach ($result["records"] as $_product) {
                    ...
                    if (!$product) {
                        ...
                        if (!$product->getName()) {
                            ...
                            Mage::helper('Erply')
                                ->log("Creating new product: " . $_product["productID"]);
                        } else {
                            Mage::helper('Erply')
